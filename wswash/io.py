...
class SwashIO(object):
    ...
    def def_waves(self, n_subset, subset):
        'Create waves Dataframe from wave parameters and water level'
        gamma = self.proj.gamma
        warmup = self.proj.warmup
        
        subset["forcing"] = ['Jonswap'] * n_subset
        subset["gamma"] = [gamma] * n_subset
        subset["warmup"] = [warmup] * n_subset
        subset["wdir"] = [0] * n_subset
        
        # Set sign of wind direction
        subset["wdir"][np.where(subset["wx"] < 0)[0]] = 180
        subset["wx"] = np.abs(subset["wx"])
    ...
    def cal_step(self, ws):
        ...
        Ls,ks,cs = self.DispersionLonda(T, 1) 
        L,k,c = self.DispersionLonda(T, deep)                 
        dx = Ls / dxL 
    ...
    def make_input(self, p_file, id_run, ws):
        ...
        dxinp = self.proj.dxinp                       # mesh size in x-dir
        dyinp = 1                                     # mesh size in y-dir
        
        # Set Gate discharge as maximun bathymetry elevation
        Gate_Q = np.argmin(depth, axis=None, out=None) * dxinp
        ...
        xlenc = int(mxinp*dxinp)                           # length bathymetry x-dir
